...
// .wrangler/tmp/bundle-VcTqOj/checked-fetch.js
...
// .wrangler/tmp/bundle-VcTqOj/strip-cf-connecting-ip-header.js
...
app.post("/api/projects/:id/requirements", async (c) => {
  ...
  projectList[index].updatedAt = (/* @__PURE__ */ new Date()).toISOString();
  await c.env.PROJECTS_DB.put("projects", JSON.stringify(projectList));
  return c.json(newRequirement, 201);
});
app.put("/api/projects/:id/requirements/:reqId", async (c) => {
  const id = c.req.param("id");
  ...
  projectList[projectIndex].updatedAt = (/* @__PURE__ */ new Date()).toISOString();
  await c.env.PROJECTS_DB.put("projects", JSON.stringify(projectList));
  return c.json(projectList[projectIndex].requirements[reqIndex]);
});
app.delete("/api/projects/:id/requirements/:reqId", async (c) => {
  const id = c.req.param("id");
  const reqId = c.req.param("reqId");
  const projects = await c.env.PROJECTS_DB.get("projects");
  const projectList = projects ? JSON.parse(projects) : [];
...
// .wrangler/tmp/bundle-VcTqOj/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-VcTqOj/middleware-loader.entry.ts
